...
def save_game_data(game_data, json_filename, titleid):
    ...
    os.makedirs(f'Games/{titleid}', exist_ok=True) # Creates a directory for the game

    # Check if the game data has already been scraped
    if not os.path.exists(json_filename):   
        # Save the XML of game as JSON
        with open(f'Games/{titleid}/{titleid}.json', 'w', encoding='utf-8') as f:
            json.dump(game_data, f, ensure_ascii=False, indent=4)

    # Save necessary Artwork
    if DOWNLOAD_ARTWORK == True:   
        # Background
        if game_data['artwork']['background'] is not None and not find_image("background", f'Games/{titleid}/'):
            save_image(game_data['artwork']['background'], 'background',f'Games/{titleid}/background')
        # Banner
        if game_data['artwork']['banner'] is not None and not find_image("banner", f'Games/{titleid}/'):
            save_image(game_data['artwork']['banner'], 'banner',f'Games/{titleid}/banner')
        # Boxart
        if game_data['artwork']['boxart'] is not None and not find_image("boxart", f'Games/{titleid}/'):
            save_image(game_data['artwork']['boxart'], 'boxart',f'Games/{titleid}/boxart')
        # Icon
        if game_data['artwork']['icon'] is not None and not find_image("icon", f'Games/{titleid}/'):
            save_image(game_data['artwork']['icon'], 'icon',f'Games/{titleid}/icon')
        # Slideshow/Gallery
        """if game_data['artwork']['gallery'] is not None and len(game_data['artwork']['gallery']) > 0:
            os.makedirs(f'Games/{titleid}/Gallery', exist_ok=True) # Creates a directory for the game's slideshow
            for slideshow_image in game_data['artwork']['gallery']:
                if not os.path.exists(f'Games/{titleid}/Gallery/{os.path.basename(slideshow_image)}'):
                    save_image(slideshow_image, f'Games/{titleid}/Gallery/{os.path.basename(slideshow_image)}')"""
...
def scrape_game_data(base_games_list):
    ...
    for game in base_games_list[:10]:
        ...
        json_filename = f"Games/{titleid}/{titleid}.json"
...
if __name__ == "__main__":
    ...
    with open(f'games.json', 'w', encoding='utf-8') as f:
